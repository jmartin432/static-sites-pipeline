...
def handler(event, context):
    ...
    print('received event from CodeBuild, project: %s, id: %s' % (project, event_id))
    ...
    print('downloading %s from %s' % (artifact_key, artifacts_bucket))
    artifacts_bucket.download_fileobj(artifact_key, artifact_zip)

    print('uploading %s to %s' % (app_name, deployment_bucket))
    ...
    print('uploading %s complete' % app_name)
